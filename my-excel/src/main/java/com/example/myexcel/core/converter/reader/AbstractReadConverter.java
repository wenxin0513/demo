...
public abstract class AbstractReadConverter<R> implements ReadConverter<String, R> {
    ...
    @Override
    public R readConvert(Field field, String obj) {
        if (StringUtil.isBlank(obj)) {
            return null;
        }
        String trimContent = obj.trim();
        // negative
            if (StringUtils.isNotBlank(trimContent) && trimContent.endsWith(Constants.RIGHT_BRACKET) && trimContent.startsWith(Constants.LEFT_BRACKET)) {
    ...
    protected String getDateFormatPattern(Field field) {
        ...
        if (Objects.nonNull(excelColumn) && StringUtils.isNotBlank(excelColumn.dateFormatPattern())) {
     ...
     * @return true/false
     */
    protected boolean isValidDate(Field field) {
        ExcelColumn excelColumn = field.getAnnotation(ExcelColumn.class);
        if (Objects.nonNull(excelColumn) && Objects.nonNull(excelColumn.isValidDate())) {
            return excelColumn.isValidDate();
        }
        return flag;
    }
